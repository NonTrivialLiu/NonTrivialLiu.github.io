...
const listFiles = (relPath) => {
  ...
  if (!fs.statSync(fullPath).isDirectory()) return [relPath];
...
const acknowledgedOverrides = new Set([...overrideManifest.matchAll(/^  ([^:\n]+):$/gm)].map((match) => match[1]));

for (const forbiddenPath of ["_includes", "_layouts", "_sass", "_scripts", "assets/tailwind", "tailwind.config.js", "assets/webfonts"]) {
  if (exists(forbiddenPath)) {
    const localFiles = listFiles(forbiddenPath);
    if (localFiles.length > 0 && localFiles.every((file) => acknowledgedOverrides.has(file))) continue;
