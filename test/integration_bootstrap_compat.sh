...
bundle exec jekyll build --config "_config.yml,test/polyglot_config.yml" -d "${site_no_compat}" >/dev/null
...
bundle exec jekyll build --config "_config.yml,test/polyglot_config.yml,${override_file}" -d "${site_with_compat}" >/dev/null
